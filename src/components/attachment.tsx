  ...
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        >
          <path strokeLinecap="round" strokeLinejoin="round" d={typeIconUrl} />
        </svg>
        <input
          type="text"
          id={type}
          placeholder={`${typeKorean} URL을 입력해주세요.`}
          className="w-full pl-3 pr-1 py-1 border-x border-dashed text-sm focus:outline-none"
          {...register(type, {
            pattern: {
              value: regex,
              message: `※ 올바른 ${typeKorean} URL을 입력해주세요. ${type === "youtube" && "(Shorts 업로드 불가)"
                }`,
            },
